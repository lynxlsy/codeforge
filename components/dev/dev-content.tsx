...
import { getAllProjects, Project, assumeProject, generateProjectReceipt, deleteProject, deleteAllProjects } from "@/lib/projects"
...
export function DevContent() {
  ...
  const handleDeleteAllProjects = async () => {
    if (!confirm('⚠️ ATENÇÃO: Isso excluirá TODOS os projetos! Tem certeza?')) return
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-black to-gray-900 relative overflow-hidden">
      ...
      <header className="relative z-10 bg-black/80 backdrop-blur-xl border-b border-gray-800 px-4 sm:px-6 lg:px-8 py-4">
        <div className="max-w-7xl mx-auto flex flex-col sm:flex-row items-start sm:items-center justify-between space-y-3 sm:space-y-0">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h1 className="text-lg sm:text-xl lg:text-2xl font-bold bg-gradient-to-r from-red-400 via-red-500 to-red-600 bg-clip-text text-transparent">
                Área do Desenvolvedor
              </h1>
              <p className="text-xs text-red-400 hidden sm:block">Dashboard avançado e gerenciamento</p>
            </div>
          </div>
      ...
      <div className="relative z-10 max-w-7xl mx-auto p-4 sm:p-6 lg:p-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
          <Card className="bg-gradient-to-br from-red-600/10 to-red-800/10 border border-red-600/20 backdrop-blur-sm">
            <CardHeader className="pb-2">
              <CardTitle className="text-sm font-medium text-red-300">Total de Funcionários</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="text-2xl font-bold text-red-400">{employees.length}</div>
            </CardContent>
          </Card>
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
          {/* Desktop Tabs */}
          <TabsList className="hidden md:grid md:grid-cols-6 w-full bg-gray-800/50 border border-gray-700/50">
            <TabsTrigger value="projects" className="data-[state=active]:bg-red-600/20 data-[state=active]:text-red-300">
            ...
            <TabsTrigger value="pricing" className="data-[state=active]:bg-blue-600/20 data-[state=active]:text-blue-300">
              <DollarSign className="w-4 h-4 mr-2" />
              Preços
            </TabsTrigger>
            <TabsTrigger value="contacts" className="data-[state=active]:bg-purple-600/20 data-[state=active]:text-purple-300">
              <Phone className="w-4 h-4 mr-2" />
              Contatos
            </TabsTrigger>
            <TabsTrigger value="labels" className="data-[state=active]:bg-cyan-600/20 data-[state=active]:text-cyan-300">
              <Tag className="w-4 h-4 mr-2" />
              Etiquetas
            </TabsTrigger>
            <TabsTrigger value="optimization" className="data-[state=active]:bg-orange-600/20 data-[state=active]:text-orange-300">
          ...
          <div className="md:hidden">
            <div className="relative">
              ...
              >
                ...
                <option value="pricing" className="bg-gray-800 text-gray-200 py-2">💰 Preços</option>
                ...
                {loading ? (
                  ...
                  <div className="text-center py-8">
                    ...
                    {projects.map((project) => (
                      <div 
                        key={project.id}
                        className="relative p-4 md:p-6 rounded-xl border border-gray-700/50 bg-gradient-to-br from-gray-800/20 to-gray-900/20 hover:border-red-600/30 hover:shadow-lg hover:shadow-red-600/10 transition-all duration-300 backdrop-blur-sm"
                      >
                        {/* Header com ícone e status */}
                        <div className="flex items-start justify-between mb-4 md:mb-6">
                          <div className="flex items-center space-x-3 flex-1 min-w-0">
                            <div className="w-8 h-8 md:w-12 md:h-12 bg-gradient-to-br from-red-600/20 to-red-700/20 rounded-xl flex items-center justify-center flex-shrink-0">
                              <Package className="w-4 h-4 md:w-6 md:h-6 text-red-400" />
                            </div>
                        ...
                        <div className="space-y-2 md:space-y-4 mb-4 md:mb-6">
                         ...
                         <div className="flex flex-wrap gap-2 md:gap-3 pt-4 md:pt-5 border-t border-gray-700/50">
                           ...
                           >
                             <Eye className="w-3 h-3 md:w-4 md:h-4 mr-1 md:mr-2 flex-shrink-0" />
                           ...
                           <Button
                             ...
                             size="sm"
                             className="flex-1 min-w-0 bg-gradient-to-r from-purple-600/20 to-purple-700/20 hover:from-purple-600/30 hover:to-purple-700/30 text-purple-300 border border-purple-600/30 text-xs md:text-sm font-medium transition-all duration-200 h-9 md:h-10 px-3 md:px-4"
                           >
                           ...
                           <Button
                             ...
                             size="sm"
                             className="flex-1 min-w-0 bg-gradient-to-r from-red-600/20 to-red-700/20 hover:from-red-600/30 hover:to-red-700/30 text-red-300 border border-red-600/30 text-xs md:text-sm font-medium transition-all duration-200 h-9 md:h-10 px-3 md:px-4"
                           >
                             <Trash2 className="w-3 h-3 md:w-4 md:h-4 mr-1 md:mr-2 flex-shrink-0" />
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-gray-900 border border-gray-700 rounded-lg p-6 max-w-2xl w-full max-h-[80vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-semibold text-gray-200">Detalhes do Projeto</h3>
              <Button
            ...
            <div className="space-y-4">
              <div>
                <h4 className="font-medium text-gray-300 mb-2">Informações do Projeto</h4>
                <div className="grid grid-cols-2 gap-4 text-sm">
                  <div>
                    <span className="text-gray-400">Título:</span>
                    <p className="text-gray-200">{selectedProject.title}</p>
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <p className="text-gray-200">R$ {selectedProject.budget?.toLocaleString('pt-BR') || '0'}</p>
                  </div>
                  <div>
                    <span className="text-gray-400">Prioridade:</span>
                    <p className="text-gray-200">{selectedProject.priority}</p>
                  </div>
                  {selectedProject.assignedTo && (
                    <div>
                      <span className="text-gray-400">Atribuído para:</span>
                      <p className="text-gray-200">{selectedProject.assignedTo}</p>
                    </div>
                  )}
                </div>
              </div>
              ...
              <div>
                <h4 className="font-medium text-gray-300 mb-2">Informações do Cliente</h4>
                <div className="grid grid-cols-2 gap-4 text-sm">
                  <div>
                    <span className="text-gray-400">Nome:</span>
                    <p className="text-gray-200">{selectedProject.clientName}</p>
                  </div>
                  <div>
                    <span className="text-gray-400">Email:</span>
                    <p className="text-gray-200">{selectedProject.clientEmail}</p>
                  </div>
                  <div>
                    <span className="text-gray-400">Telefone:</span>
                    <p className="text-gray-200">{selectedProject.clientPhone}</p>
                  </div>
                  <div>
                    <span className="text-gray-400">Data de Criação:</span>
                    <p className="text-gray-200">{selectedProject.createdAt}</p>
                  </div>
                </div>
              </div>
              ...
              {selectedProject.description && (
                <div>
                  <h4 className="font-medium text-gray-300 mb-2">Descrição</h4>
                  <p className="text-gray-200 text-sm">{selectedProject.description}</p>
                </div>
              )}
              
              {selectedProject.requirements && selectedProject.requirements.length > 0 && (
                <div>
                  <h4 className="font-medium text-gray-300 mb-2">Requisitos</h4>
