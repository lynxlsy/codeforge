...
// Template para ícones iOS otimizados
const iosIconTemplate = (size) => `<?xml version="1.0" encoding="UTF-8"?>
<svg width="${size}" height="${size}" viewBox="0 0 ${size} ${size}" xmlns="http://www.w3.org/2000/svg">
  ...
  <!-- Background sem bordas arredondadas para iOS -->
  <rect width="${size}" height="${size}" fill="url(#bg)"/>
  
  <!-- Logo CDforge centralizado -->
  <g transform="translate(${size/2}, ${size/2}) scale(${size/2300}, ${size/2300}) translate(-271.5, -283.5)">
    <g fill="url(#logo)" stroke="none">
...
const iosIconSizes = [152, 167, 180, 192];
...
iosIconSizes.forEach(size => {
  const filename = `ios-icon-${size}x${size}.svg`;
  const filepath = path.join(iconsDir, filename);
  
  fs.writeFileSync(filepath, iosIconTemplate(size));
